...
namespace VectorEditor.objects
{
    class VEllipse : Figure
    {
        ...
        public override void Draw(Graphics g) //Отрисовывание прямоугольника
        {
            ...
            {
                g.FillEllipse(new SolidBrush(color), Math.Min(one.Value.X, two.Value.X), Math.Min(one.Value.Y, two.Value.Y), Math.Abs(one.Value.X - two.Value.X), Math.Abs(one.Value.Y - two.Value.Y)); //заливаем

            }
            if (!thicknessColor.IsEmpty) //Если у нас есть цвет контура
            {
                g.DrawEllipse(new Pen(thicknessColor, thickness), Math.Min(one.Value.X, two.Value.X), Math.Min(one.Value.Y, two.Value.Y), Math.Abs(one.Value.X - two.Value.X), Math.Abs(one.Value.Y - two.Value.Y)); //рисуем контур прямоугольника
            }                                                                                                                                                                                           //
        }
    }
}
